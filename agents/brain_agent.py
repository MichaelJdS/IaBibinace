...
class BrainAgent:
    ...
    def _process_pair(self, pair: str):
        ...
        if pos:
            ...
            if sl_hit or tp_hit or tr_hit:
                ...
                if result and result[0]:
                    _, pnl_pct, pnl_usd, is_win = result
                    duration = int(time.time() - pos.get("entry_time", time.time()))

                    self.risk.record_trade(
                        pnl_usd, pnl_pct, is_win, reason,
                    ...
                    stats = self.risk.get_stats()
                    if abs(stats["daily_loss"]) >= self.risk.daily_start_bal * config.MAX_DAILY_LOSS_PCT:
                        notify_daily_loss_limit(
                            pct_used  = 100.0,
                            daily_pnl = stats["daily_loss"]
                        )
                return

        open_positions = len(self.executor.get_open_positions())
        can, reason = self.risk.can_trade(open_positions)
        if not can:
            if self._cycle % 20 == 0:
        ...
        # Detecta mudança de regime e notifica
        ...
        if decision["action"] == "BUY":
            ...
            ok = self.executor.buy(quantity=qty, price=price, symbol=pair, stop_loss=sl, take_profit=tp)
            if ok:
